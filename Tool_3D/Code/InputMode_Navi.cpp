	...
	CInputMode(_pInputModeMgr)
{
	...
	// 네비 메쉬 입력 처리기를 갖고 온다.
	m_pNaviMeshInputProcessor = g_pTool3D_Kernel->GetEditScene()->GetNaviMeshInputProcessor();
}
...
_int CInputMode_Navi::ProcessInput(const _float & _fDeltaTime)
{
	...
	if (Engine::CDirectInputMgr::GetInstance()->IsKeyDown(Engine::DIM_LB)) {
		m_pNaviMeshInputProcessor->Click();
	}
	else if (Engine::CDirectInputMgr::GetInstance()->IsKeyPressing(Engine::DIM_LB)) {
		m_pNaviMeshInputProcessor->Drag(_fDeltaTime);
	}
	else {
		m_pNaviMeshInputProcessor->Idle(_fDeltaTime);
	}
