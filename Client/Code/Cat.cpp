...
{
	// Mesh
	m_pMesh = dynamic_cast<Engine::CDynamicMesh*>(Engine::Clone(Engine::RESOURCE_STAGE, L"Cat"));
	...
	// StateMgr
	m_pStateMgr = new CStateMgr<CCat>(*this);
	m_pStateMgr->SetNextState(new CCatState_Idle(*this));
...
int CCat::Update_Object(const _float & _fDeltaTime)
{
	if (!IsActivated())
		return 1;
...
_bool CCat::LoadColliders(const _tchar* _pFileName)
{
...
//void CCat::OnCollision(Engine::CollisionInfo _tCollisionInfo)
//{
//	/*if (!m_bIsTalking) {
...
//}
//
//void CCat::OnNotCollision(Engine::CollisionInfo _tCollisionInfo)
//{
//	if (m_bIsTalking) {
//		if (lstrcmp(_tCollisionInfo.pCollidedCollider->GetColliderTag(), L"Player") == 0) {
...
CCat * CCat::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
...
//
//_bool CCat::LoadRoutePosList(const _tchar * _pRouteFilePath)
//{
//	FILE* fpIn;
//	errno_t err = 0;
//
...
void CCat::ShoutOut(_float _fOffsetX, _float _fOffsetY, const wstring & _wstrText)
{
	m_pSpeechBubble->SetText(_fOffsetX, _fOffsetY, _wstrText);
}

//_bool CCat::IsMoving(const _float & _fDeltaTime, _vec3 * _pDir)
//{
