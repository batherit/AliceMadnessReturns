                     ...
                     _In_ int       nCmdShow)
{
	...
	g_pMainApp = CMainApp::Create();
	if (nullptr == g_pMainApp)
		return FALSE;

	POINT ptOldCursor = Engine::GetClientCursorPoint(g_hWnd);
	Engine::CKeyMgr::GetInstance()->SetOldCursorPoint(ptOldCursor);
	...
	while (true)
	{
		...
		else
		{
			...
			if (Engine::IsPermit_Call(L"Frame_FPS60", fTimeDelta))
			{
				...
				g_pMainApp->Update_MainApp(fTime60);
	...
	if (dwRefCnt = Safe_Release(g_pMainApp))
	{
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	// 윈도우 창 사이즈 = 렉트 사이즈 + 기본 윈도우 창 스타일 + 메뉴 바 크기
